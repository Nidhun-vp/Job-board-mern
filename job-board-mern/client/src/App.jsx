...
export default function App() {
  ...
  async function handleSubmit(e) {
    e.preventDefault();
    const { title, company, category, location, description } = formData;
    ...
    try {
      ...
      setFormData({ title: "", company: "", category: "Design", location: "", description: "" });
      setShowForm(false);
      fetchJobs();
    } catch (err) {
      ...
      <header className="bg-blue-700 text-white w-full">
        <div className="p-4 max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between">
          ...
          <button
            ...
            className="bg-primary text-blue-700 px-5 py-2 rounded-md font-semibold hover:bg-gray-200 transition focus:ring-2 focus:ring-blue-400"
          >
            Post a Job
          </button>
        </div>
              ...
              <div key={job._id || job.id} className="col-md-6 col-lg-4 mb-4">
                <div className="card h-100 shadow-sm">
                  <div className="card-body">
                    ...
                    <span className="badge bg-danger">{job.category}</span>
                  </div>
                </div>
              </div>
            ))}
